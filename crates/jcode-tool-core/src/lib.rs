...
impl ToolContext {
    ...
    ///
    /// This matches Claude Code's `expandPath`, which handles `~` and `~/` and
    /// nothing else. Note their Read *prompt* says `file_path` "must be an
    /// absolute path": that is instruction to the model, not what the
    /// implementation accepts. Their code expands deliberately, so a permission
    /// check cannot be bypassed by passing an unexpanded `~`.
    ///
    /// Neither `~user` nor `$HOME` is expanded, also matching `expandPath`.
    /// `~user` needs a passwd lookup, and there is no shell here to make `$VAR`
    /// meaningful; expanding it would let a path argument pick up ambient state.
...
/// failing.
///
/// `~user` is deliberately *not* supported, matching Claude Code's `expandPath`:
/// resolving another user's home needs a passwd lookup, and treating it as a
/// literal is safer than guessing at a sibling of the current home.
...
mod tests {
    ...
    /// Claude Code's `expandPath` does not expand variables either, so this is
    /// the matching behavior rather than a gap: expanding them would let a path
    /// argument silently pick up ambient state.
    #[test]
