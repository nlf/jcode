        ...
        match event {
            ...
            WindowEvent::RedrawRequested => {
                ...
                if let Some(size) = self.state.as_ref().map(render::RenderState::size) {
                    ...
                    // `end_build` and `render` now happen inside the host's
                    // present callback, so a worker swap keeps every wgpu call
                    // resolving against the host's original registry.
                    let app = self as *mut App;
