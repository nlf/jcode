...
pub const DEFAULT_RULES: &[Rule] = &[
    Rule {
        pattern: r"^\s*(cat|head|tail|less|more)\s+",
        tool: "read",
        message: "Use the `read` tool instead of cat/head/tail. It returns numbered lines, which is what makes the file editable, and handles binary and PDF files.",
    },
    Rule {
